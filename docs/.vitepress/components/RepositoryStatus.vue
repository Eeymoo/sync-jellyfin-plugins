    ...
    <div v-else class="repository-grid">
      ...
      >
        <div class="repo-header">
          ...
          <div class="repo-status" :class="`status-${repo.status}`">
          </div>
        </div>
        ...
        <div class="repo-info">
          <p class="last-update">最后更新: {{ formatDate(repo.timestamp) }}</p>
          <div class="repo-url">
            <input
              :value="repo.repositoryUrl || repo.originalUrl"
              readonly
              class="url-input"
            />
            <button
        ...
        <div class="status-history">
          ...
          <div class="history-chart">
            <div
              v-for="(record, index) in repo.statusHistory?.slice(0, 60) || []"
              :key="index"
              class="history-dot"
              :class="`status-${record.status}`"
              :title="`${formatDate(record.timestamp)}: ${record.status}${record.error ? ' - ' + record.error : ''}`"
            ></div>
            <div v-if="!repo.statusHistory || repo.statusHistory.length === 0" class="no-history">
              暂无历史记录
            </div>
          </div>
          <div class="history-stats" v-if="repo.statusHistory && repo.statusHistory.length > 0">
            <span class="success-rate">
              成功率: {{ repo.successRate || calculateSuccessRate(repo.statusHistory) }}%
            </span>
            <span class="total-checks">
              总检查次数: {{ repo.statusHistory.length }}
            </span>
          </div>
          <div v-if="repo.lastError" class="last-error">
            <strong>最近错误:</strong> {{ repo.lastError }}
          </div>
        </div>
      </div>
    </div>
...
const fetchRepositories = async () => {
  try {
    ...
    error.value = '无法加载仓库数据: ' + err.message
